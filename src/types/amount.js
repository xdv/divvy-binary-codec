...
const DROPS_PER_XDV = new Decimal('1e6');
const MAX_NETWORK_DROPS = new Decimal('1e17');
const MIN_XDV = new Decimal('1e-6')
const MAX_XDV = MAX_NETWORK_DROPS.dividedBy(DROPS_PER_XDV);
...
Native values must be described in drops, a million of which equal one XDV.
...
const parsers = {
  string(str) {
    ...
    return [new Decimal(str).dividedBy(DROPS_PER_XDV), Currency.XDV];
  },
  object(object) {
...
const Amount = makeClass({
  Amount(value, currency, issuer, validate = true) {
    ...
    this.currency = currency || Currency.XDV;
  ...
  statics: {
    ...
    fromParser(parser) {
      ...
      const xdvValue = drops.dividedBy(DROPS_PER_XDV);
      return new this(xdvValue, Currency.XDV, null, false);
    }
  },
  assertValueIsValid() {
    ...
    if (!this.isZero()) {
      if (this.isNative()) {
        ...
        if (abs.lt(MIN_XDV) || abs.gt(MAX_XDV)) {
          // value is in XDV scale, but show the value in canonical json form
          raiseIllegalAmountError(this.value.times(DROPS_PER_XDV))
        }
      } else {
  ...
  valueString() {
    return (this.isNative() ? this.value.times(DROPS_PER_XDV) : this.value)
            .toString();
  },
  toBytesSink(sink) {
